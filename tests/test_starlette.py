...
async def _app(**kwargs):
    app = Starlette()
    kwargs.update({"kwargs": dict(max_inactive_connection_lifetime=_MAX_INACTIVE_CONNECTION_LIFETIME)})
    ...
    async def get_user(request):
        ...
        if method == "1":
            ...
            return JSONResponse((await request["connection"].first_or_404(q)).to_dict())
        elif method == "3":
